...
export default class GiftService {
  ...
  static async get(id: string): Promise<IResponseBody<IGift[]>> {
    try {
      const response = await api.get<IResponseBody<IGift[]>>(this.path, {
  ...
  static async getById(id: string): Promise<IResponseBody<IGift[]>> {
    try {
      const response = await api.get<IResponseBody<IGift[]>>(
        this.path + "-id",
        { params: { id_premio: id } },
      );
      return response.data;
  ...
  static async getByCategory(
    ...
    const response = await api.get<IResponseBody<any[]>>(
      `/gift?category=${categoryName}`,
    );
    return response.data;
  }
}
